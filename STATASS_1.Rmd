...
6. split feature, cv index and labels
```{r}
dim(data.final)
# save cv info
...
data.final$cv = NULL
data.final$act_demented = NULL
which(colnames(data.final)=="act_demented")
which(colnames(data.final)=="cv")
dim(data.final)
```

## feature filtering

data.final - all feature columns (without cv and y)
cv.377 - 10 folds cross-validation index
y - labels
...
3. remove features with t test by class
```{r}
data.byClass <- split(data.filter2, y)
...
for (pc in seq(30,300, length=10)) {
  lr.acc.train <- c()
  lr.acc.test <- c()
